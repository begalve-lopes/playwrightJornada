import { Locator, Page, expect } from "@playwright/test"; // Importado o 'expect' aqui
...
export default class PaginaPrincipal {
    ...
    private readonly boataoDefinirPassagemExecutiva: Locator;
    ...
    constructor(page: Page) {
        ...
        this.boataoDefinirPassagemExecutiva= page.getByTestId('botao-passagem-executiva');
    ...
    async definirSomenteIda() {
        await this.botaoSomenteIda.click(); // Adicionado o await ausente
    }
    ...
    async definirPassagemExecutiva(){
        await this.boataoDefinirPassagemExecutiva.click()
    }

    async fecharModalPassageiro() {
        await this.botaoFecharModalPassageiros.click();
    }

    async definirOrigemEdestino(origin: string, destino: string) {
        await this.campoDropdownOrigem.fill(origin);
    ...
    async estaMostarandoPassagem(tipoTrajeto: 'Somente ida' | 'Ida e volta', origem: string, destino: string) {
